...
<div class="content-header">
	<div class="container-fluid">
		<div class="row mb-2">
			<div class="col-sm-6">
				<h1 class="m-0 text-dark">Pregnant List</h1>
			</div>
			<div class="col-sm-6">
				<ol class="breadcrumb float-sm-right">
					<li class="breadcrumb-item"><a href="#">Home</a></li>
					<li class="breadcrumb-item active">Pregnants</li>
				</ol>
			</div>
		</div>
	</div>
			...
			<div class="col-12 mx-auto">
				<div class="card card-primary card-outline">
					<div class="card-header">
						<h3 class="card-title">Mother with prenatal records</h3>
						...
						<div class="table-responsive mailbox-messages">
							<table class="table tbl-inbox">
								<thead>
									...
									<th style="width: 100px" class="text-nowrap">Pregnancy Status</th>
								</thead>
...
<div class="modal fade" data-backdrop="static" id="modal-add-pregnant">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Add New Pregnant</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form action="" method="post" id="add-pregnant-form">
        	<style type="text/css">
            ...
            <li class="nav-item">
              <a class="nav-link active" href="#mother-tab" data-toggle="tab">Mother</a>
            </li>
            <li class="nav-item">
              <a class="nav-link" href="#father-tab" data-toggle="tab">Father</a>
            </li>
          </ul>
							...
							<div class="row">
								<div class="col-sm-6 col-md-6 col-lg-6">
									<div class="form-group">
										...
										<input type="number" name="weight" class="form-control" min="10" step="1" max="100">
									</div>
								</div>
								<div class="col-sm-6 col-md-6 col-lg-6">
									<div class="form-group">
										<label>Last Period</label>
										<input type="date" name="last-period" value="" class="form-control">
									</div>
								</div>
							</div>
						</div>
      ...
      <div class="modal-footer">
        <div class="float-right">
	        <button type="submit" form="add-pregnant-form" name="save-continue" value="" class="btn btn-primary btn-sm">
...
<script type="text/javascript">
	$(function() {
		...
		$(document).on('click', '.btn-add-pregnant', function() {
			$('#add-pregnant-form').find('input, select').each(function(i, elem) {
				$(elem).val('');
				$(elem).removeClass('is-invalid').closest('.form-group').find('span.error:first').remove();
			})
			$('#modal-add-pregnant').modal('show');
		})

		const getPregnants = function(pathURL="{{ route('pregnants.list') }}") {
			$.loading.show();
			var q = $('[name=q]').val();
			$.ajax({
				method: 'POST',
						...
						for(var i=0; i<resp.data.length; i++) {
							htmlRow += `
								<tr>
									...
									<td class="mailbox-name" style="min-width: 150px; width: 130px;">
										<span class="text-overflow-dynamic-container">
											<span class="text-overflow-dynamic-ellipsis">
												<a href="${'{{ route("pregnants.show", ["pregnant"=>"@pregnant"]) }}'.replace('@pregnant', resp.data[i].mother_no)}" target="_blank">
									...
									<td>${getAge(resp.data[i].birthdate)}</td>
									<td class="mailbox-subject text-nowrap" style="min-width: 300px;">
										<span class="text-overflow-dynamic-container">
											<span class="text-overflow-dynamic-ellipsis">
													${resp.data[i].address} 
											</span>
									...
									<td class="mailbox-date" style="width: 60px;">
										${resp.data[i].has_prenatal?
											'<small><i><i class="fas fa-calendar"></i> Has Schedule</i></small>':
											'<small><i><i class="fas fa-times"></i> No Prenatal</i></small>'
										}
									</td>
								</tr>
							`;
		...
		$(document).on('change', '#add-pregnant-form input, #add-pregnant-form select', function() {
		...
		$(document).on('submit', '#add-pregnant-form', function(e) {
			e.preventDefault();
			...
			$.ajax({
				...
				success: function(resp) {
					...
					if (resp.statusCode==419) {
						...
						$('#modal-add-pregnant').modal('hide');
						toastr.success('Success to add new pregnant record.');
						getPregnants();
					}
					$.loading.hide();
				},
